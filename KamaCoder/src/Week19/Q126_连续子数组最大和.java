 ...
 * 1. 状态表示
 * 集合：
 * f[i][0]为未使用替换操作时，[0, i]中元素可构成的连续子数组最大值
 * f[i][1]为使用了替换操作时，[0, i]中元素可构成的连续子数组最大值
 * 属性：Max
 * 2. 状态计算
 * 初始状态：
 * f[0][0] = nums[0]
 ...
 * f[i][1] = max(f[i - 1][0] + x, f[i - 1][1] + nums[i], x)
 */
public class Q126_连续子数组最大和 {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            f[0][0] = nums[0];
            f[0][1] = x;
            int ans = Math.max(f[0][0], f[0][1]);
            for (int i = 1; i < n; i++) {
                f[i][0] = Math.max(f[i - 1][0] + nums[i], nums[i]);
                f[i][1] = Math.max(f[i - 1][0] + x, Math.max(f[i - 1][1] + nums[i], x));
